//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <objc/NSObject.h>

@class UIView;

@interface WAContainerView : NSObject
{
    _Bool _cached;
    _Bool _b_mIsNativeViewAdd2ScrollView;
    UIView *_scrollView;
    UIView *_nativeView;
}

@property(nonatomic) _Bool b_mIsNativeViewAdd2ScrollView; // @synthesize b_mIsNativeViewAdd2ScrollView=_b_mIsNativeViewAdd2ScrollView;
@property(nonatomic) _Bool cached; // @synthesize cached=_cached;
@property(nonatomic) __weak UIView *nativeView; // @synthesize nativeView=_nativeView;
@property(nonatomic) __weak UIView *scrollView; // @synthesize scrollView=_scrollView;
- (void).cxx_destruct;
- (void)dealloc;
- (id)hitTest:(struct CGPoint)arg1 withEvent:(id)arg2;
- (void)updateContainer;

@end

