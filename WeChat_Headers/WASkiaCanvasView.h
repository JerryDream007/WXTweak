//
//     Generated by class-dump 3.5 (64 bit) (Debug version compiled Sep 17 2017 16:24:48).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2015 by Steve Nygard.
//

#import <UIKit/UIView.h>

#import "CanvasViewDelegate-Protocol.h"

@class CAEAGLLayer, EAGLContext, NSDictionary, NSMutableDictionary, NSString, UIImageView;
@protocol WASkiaViewDelegate;

@interface WASkiaCanvasView : UIView <CanvasViewDelegate>
{
    CAEAGLLayer *_eaglLayer;
    struct BindingApp *_app;
    unsigned int _framebuffer;
    unsigned int _renderbuffer;
    long long _touchId;
    NSMutableDictionary *_touches;
    NSDictionary *_longpressParam;
    _Bool _touched;
    UIImageView *_snapshotImageView;
    unsigned int _canvasId;
    int _gestureMode;
    id <WASkiaViewDelegate> _delegate;
    NSString *_userData;
    EAGLContext *_context;
    shared_ptr_3789f237 _canvasView;
}

+ (id)getVirsionString;
+ (Class)layerClass;
@property(retain, nonatomic) EAGLContext *context; // @synthesize context=_context;
@property(nonatomic) int gestureMode; // @synthesize gestureMode=_gestureMode;
@property(nonatomic) unsigned int canvasId; // @synthesize canvasId=_canvasId;
@property(retain, nonatomic) NSString *userData; // @synthesize userData=_userData;
@property(nonatomic) __weak id <WASkiaViewDelegate> delegate; // @synthesize delegate=_delegate;
@property(nonatomic) shared_ptr_3789f237 canvasView; // @synthesize canvasView=_canvasView;
- (id).cxx_construct;
- (void).cxx_destruct;
- (void)setSkiaCanvasTextureDelegate:(id)arg1;
- (id)getUniqueIdAppendAppTag;
- (void)setFileDelegate:(id)arg1;
- (void)touchesCancelled:(id)arg1 withEvent:(id)arg2;
- (void)touchesEnded:(id)arg1 withEvent:(id)arg2;
- (void)touchesMoved:(id)arg1 withEvent:(id)arg2;
- (void)onLongPress:(id)arg1;
- (void)touchesBegan:(id)arg1 withEvent:(id)arg2;
- (long long)getTouchID:(id)arg1 remove:(_Bool)arg2;
- (id)hitTest:(struct CGPoint)arg1 withEvent:(id)arg2;
- (void)setCustomSize:(struct CGSize)arg1;
- (void)snapshotEnd;
- (void)snapshotBegin;
- (id)getImage;
- (void)setBindingApp:(struct BindingApp *)arg1 canvasId:(unsigned int)arg2;
- (void)present;
- (void)makeCurrentContext;
- (struct CGSize)getFrameBufferSize;
- (unsigned int)getFrameBufferId;
- (void)initSkia:(unsigned int)arg1 size:(struct CGSize)arg2 app:(struct BindingApp *)arg3;
- (void)dealloc;
- (id)initWithFrame:(struct CGRect)arg1 contentScale:(float)arg2 bindingApp:(struct BindingApp *)arg3 andCanvasId:(int)arg4;

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

